...
printf("Got UDP packet from %s:%d (%d bytes)\n",
ipaddr_ntoa(addr), port, p->tot_len);

// Print payload
char buf[128] = {0};
pbuf_copy_partial(p, buf, LWIP_MIN(p->tot_len, sizeof(buf) - 1), 0);
printf("Payload: %s\n", buf);
...
err_t err = udp_bind(pcb, IP6_ADDR_ANY, 5678);  // Bind to port 5678
if (err != ERR_OK) {
...
printf("UDP listener on port 5678 ready\n");
}
...
void setup_tcp_listener(void) {
    ...
    err_t err = tcp_bind(pcb, IP6_ADDR_ANY, 8080); // Listen on port 8080
    if (err != ERR_OK) {
        printf("TCP bind failed: %d\n", err);
        return;
    }

    pcb = tcp_listen(pcb); // Transition into listening state
    if (!pcb) {
    ...
    printf("TCP listener on port 8080 ready\n");
}
